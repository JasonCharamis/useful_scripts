...
for $og ( keys %file ) {
	...
	$universal_single_copy{$og} = 1;
    }
}

 
#if orthogroup is found in all or all-but-two species save the ids into the universal dictionary    
...
#orthogroups widespread (all or all-but-two) in phlebotominae    
...
#orthogroups specific (all) to lutzomyia genus 
...
#orthogroups present only in phlebotomines, and with patchy distribution
foreach ( sort keys %phlebotominae ) {
    unless ( exists $universal_single_copy{$_} ) {
	unless ( exists $universal{$_} ) {
	    unless ( exists $species_specific{$_} ) {
...
#orthogroups present in nematocera (phlebotomines and mosquitoes and midges), with patchy distribution
foreach ( sort keys %phlebotominae ) {
    unless ( exists $universal_single_copy{$_} ) {
	unless ( exists $universal{$_} ) {
	    unless ( exists $species_specific{$_} ) {
		unless ( exists $brachycera{$_} ) {
			...
			unless ( exists $phlebotominae_patchy{$_} ) {
			    unless ( exists $phlebotomus_specific{$_} ) {
				unless ( exists $lutzomyia_specific{$_} ) {
				    if ( $species_counter{$_} >= 2  && $species_counter{$_} < 17 ) {
					$nematocera_patchy{$_} = $phlebotominae{$_};
				    }
				}
			    }
			}
...
#orthogroups present in diptera, with patchy distribution
foreach ( sort keys %phlebotominae ) {
    unless ( exists $universal_single_copy{$_} ) {
	unless ( exists $universal{$_} ) {
	    unless ( exists $species_specific{$_} ) {
		unless ( exists $nematocera_patchy{$_} ) {
			...
			unless ( exists $phlebotominae_patchy{$_} ) {
				...
				unless ( exists $lutzomyia_specific{$_} ) {
					...
					$diptera_patchy{$_} = $phlebotominae{$_};
				    }
				}
			    }
			}
...
print OUT0 "Species\tUniversal_Single_Copy\tUniversal\tPhlebotominae_wide\tPhlebotominae_patchy\tPhlebotomus_specific\tLutzomyia_specific\tNematocera_Patchy\tBrachycera_Patchy\tSpecies_specific\n";
...
for my $sp ( sort keys %data ) {
    ...
    for my $num (0..scalar( @number_universal_single_copy))  {
    ...
    print OUT0 "$sp\t$sum_uni_single_copy\t$sum_uni\t$sum_phlw\t$sum_phl_patchy\t$sum_phls\t$sum_ltz\t$sum_nematocera_patchy\t$sum_diptera_patchy\t$sum_spsp\n";
